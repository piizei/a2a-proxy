...
class TopicManager:
        ...
        try:
            ...
            if self.connection_string:
                self._admin_client = await loop.run_in_executor(
                    self._executor,
                    lambda: ServiceBusAdministrationClient.from_connection_string(
                ...
                self._admin_client = await loop.run_in_executor(
                    self._executor,
                    lambda: ServiceBusAdministrationClient(
    ...
    def _create_topic_properties(self, topic_name: str, config: TopicGroupConfig) -> TopicProperties:
        """Create topic properties from group configuration.
        ...
        Returns:
            TopicProperties for Service Bus
        """
        return TopicProperties(
            name=topic_name,
            max_size_in_megabytes=config.max_message_size_mb * 1024,  # Convert MB to bytes
    ...
    async def _create_single_topic(self, topic_name: str, config: TopicGroupConfig) -> TopicOperationResult:
        ...
        try:
            ...
            try:
                ...
                needs_update = (
                    existing_topic.max_size_in_megabytes != config.max_message_size_mb * 1024 or
                    existing_topic.default_message_time_to_live != timedelta(seconds=config.message_ttl_seconds) or
                    existing_topic.duplicate_detection_history_time_window != timedelta(minutes=config.duplicate_detection_window_minutes)
                )
                
                if needs_update:
                    # Create new properties for update
                    updated_properties = TopicProperties(
                        name=topic_name,
                        max_size_in_megabytes=config.max_message_size_mb * 1024,
                        default_message_time_to_live=timedelta(seconds=config.message_ttl_seconds),
                        duplicate_detection_history_time_window=timedelta(minutes=config.duplicate_detection_window_minutes),
                        enable_partitioning=existing_topic.enable_partitioning,  # Can't change after creation
                        enable_express=existing_topic.enable_express,
                        support_ordering=existing_topic.support_ordering,
                        requires_duplicate_detection=existing_topic.requires_duplicate_detection,
                        enable_batched_operations=True
                    )
                ...
                properties = self._create_topic_properties(topic_name, config)
                await self._retry_operation(
    ...
    async def list_managed_topics(self) -> List[str]:
        ...
        try:
            def _list_topics():
                return [topic.name for topic in self._admin_client.list_topics() if topic.name.startswith("a2a.")]
            
            all_topics = await self._retry_operation(_list_topics)
            logger.debug(f"Found {len(all_topics)} managed topics")
