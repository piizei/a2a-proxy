...
class TopicManager:
    ...
    def _get_topic_properties(self, config: TopicGroupConfig) -> Dict[str, Any]:
        """Get topic properties for a group configuration (for testing/inspection).
        
        Args:
            config: Agent group configuration
            
        Returns:
            Dictionary of topic properties that would be used for creation
        """
        return self._create_topic_properties(config, f"a2a.{config.name}.requests")
    
    def _create_topic_properties(self, config: TopicGroupConfig, topic_name: str) -> Dict[str, Any]:
    ...
    async def _retry_operation(self, operation: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        raise A2AProxyError("Operation failed without exception")
    ...
    async def _create_single_topic(self, topic_name: str, config: TopicGroupConfig) -> TopicOperationResult:
        ...
        try:
            ...
            try:
                ...
                topic_kwargs = self._create_topic_properties(config, topic_name)
